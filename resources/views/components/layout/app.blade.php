            ...
            <ul class="nav navbar-nav text-light" id="accordionSidebar">
                ...
                <x-navigation.navigation-section>
                    ...
                    @if (hasPermission('manage_staff_applications') &&
                            hasTable('website_staff_applications') &&
                            config('habbo.core.using_atom_cms'))
                        <x-navigation.navigation-item
                ...
                @if (hasPermission('manage_badges'))
                    <x-navigation.navigation-section>
                        ...
                        @if (hasPermission('manage_badges') && hasTable('users_badges'))
                            <x-navigation.navigation-item
                                :classes="request()->routeIs('manage-badges.*') ? 'active' : ''">
                                <a href="{{ route('manage-badges.index') }}">
                ...
                @if (hasPermission('manage_website_settings') ||
                        hasPermission('manage_website_whitelists') ||
                        hasPermission('manage_website_blacklists') ||
                    ...
                    <x-navigation.navigation-section>
                        ...
                        @if (hasPermission('manage_website_settings') && hasTable('website_settings') && config('habbo.core.using_atom_cms'))
                            <x-navigation.navigation-item
                                :classes="request()->routeIs('website-settings.*') ? 'active' : ''">
                        ...
                        @if (hasPermission('manage_website_whitelists') &&
                                hasTable('website_ip_whitelist') &&
                                config('habbo.core.using_atom_cms'))
                            <x-navigation.navigation-item
                                :classes="request()->routeIs('website-whitelist.*') ? 'active' : ''">
                        ...
                        @if (hasPermission('manage_website_blacklists') &&
                                hasTable('website_ip_blacklist') &&
                                config('habbo.core.using_atom_cms'))
                            <x-navigation.navigation-item
                                :classes="request()->routeIs('website-blacklist.*') ? 'active' : ''">
                ...
                @if (hasPermission('manage_emulator_settings') || hasPermission('manage_emulator_texts') || hasPermission('manage_external_texts'))
                    <x-navigation.navigation-section>
                        <x-slot:header>
                            {{__('Emulator')}}
                        </x-slot:header>
                        ...
                        @if (hasPermission('manage_external_texts'))
                            <x-navigation.navigation-item
                                :classes="request()->routeIs('external-texts.*') ? 'active' : ''">
                                <a href="{{ route('external-texts.index') }}">
                                    <x-icons.text-icon/>
                                    <span>{{__('External texts')}}</span>
                                </a>
                            </x-navigation.navigation-item>
                        @endif
                    </x-navigation.navigation-section>
                    ...
                    <ul class="nav navbar-nav flex-nowrap ml-auto">
                        <li class="nav-item dropdown d-sm-none no-arrow">
                            ...
                            <div class="dropdown-menu dropdown-menu-right p-3 animated--grow-in" role="menu"
                                ...
                                <form class="form-inline mr-auto navbar-search w-100" method="GET"
        ...
        <footer class="bg-white sticky-footer">
            <div class="container my-auto">
                <div class="text-center my-auto copyright">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" style="height: 15px; width: 15px; color: red;"
